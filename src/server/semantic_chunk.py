# -*- coding: utf-8 -*-
# 告诉 Python 解释器，该文件使用 UTF-8 编码，确保中文等特殊字符能正确显示
...
# 手动触发垃圾回收，释放不再使用的对象占用的内存
gc.collect()
# 清空 PyTorch 在 GPU 上的缓存
torch.cuda.empty_cache()
# 检查是否有可用的 GPU，如果有则使用 GPU，否则使用 CPU
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# 定义最小块大小，用于合并误切分的小块
_min_chunk_size = 50
# 定义最小文档大小，当输入文档长度小于该值时直接返回
_min_doc_size = 256 

# 定义一个异步上下文管理器，用于管理 FastAPI 应用的生命周期
@asynccontextmanager
async def lifespan(app: FastAPI):
    """
    一个异步上下文管理器，用于管理 FastAPI 应用的生命周期。
    确保在应用生命周期结束后清空 GPU 内存。
    """
    # 执行 yield 语句，将控制权交给调用方，开始应用的生命周期
    yield
    # 检查是否有可用的 GPU
...
# 创建一个 FastAPI 应用实例，并指定生命周期管理函数
app = FastAPI(lifespan=lifespan)

# 为 FastAPI 应用添加跨域资源共享中间件
app.add_middleware(
    CORSMiddleware,
    # 允许所有来源的请求
    allow_origins=["*"],
...
# 定义一个请求模型类，继承自 BaseModel，用于验证和序列化请求数据
class SemanticRequest(BaseModel):
    # 定义一个字符串类型的字段，用于存储待分组的句子
    sentences: str
    # 定义一个可选的整数类型字段，默认值为 5，用于指定每组的目标最大句子数
    group_size: Optional[int] = 5

# 定义一个响应模型类，继承自 BaseModel，用于验证和序列化响应数据
class ChunkResponse(BaseModel):
    # 定义一个字符串列表类型的字段，用于存储合并后的分组文本列表
    chunks: List[str]

# 定义一个 POST 请求接口，路径为 /v1/semantic-chunks，指定响应模型
...
async def create_chat_completion(request: SemanticRequest):
    ...
    # 生成句子的嵌入向量，使用预训练模型进行编码，已自动使用 GPU 加速
...
if __name__ == "__main__":
    ...
    # 使用 uvicorn 运行 FastAPI 应用，指定主机、端口和工作进程数
